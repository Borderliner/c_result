...
Result process_file(const char* filename) {
    ...
    fclose(file);
    return make_success_string(buffer);
}

int main() {
    const Result res = process_file("test.txt");
    if (is_success(res)) {
        printf("Operation successful: %s\n", ACCESS_RESULT_VALUE(res, char));
        free_result(res, char);
    } else {
        fprintf(stderr, "Operation failed: %s (code: %d)\n", res.data.message ? res.data.message : "No message", res.code);
    }
}
